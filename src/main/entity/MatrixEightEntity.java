package main.entity;

import java.util.Collections;

import java.util.ArrayList;
import java.util.List;
public class MatrixEightEntity {

	    private Integer idMatrix;
	    private Integer a11;
	    private Integer a12;
	    private Integer a13;
	    private Integer a21;
	    private Integer a22;
	    private Integer a23;
	    private Integer a31;
	    private Integer a32;
	    private Integer a33;
	    
	    public MatrixEightEntity() {
	    }
	    
	    public MatrixEightEntity(final Integer a11, final Integer a12, final Integer a13, final Integer a21, final Integer a22, final Integer a23, final Integer a31, final Integer a32, final Integer a33) {
	        this.a11 = a11;
	        this.a12 = a12;
	        this.a13 = a13;
	        this.a21 = a21;
	        this.a22 = a22;
	        this.a23 = a23;
	        this.a31 = a31;
	        this.a32 = a32;
	        this.a33 = a33;
	    }
	    
	    public List<Integer> getNumbers() {
	        final List<Integer> plays = new ArrayList<Integer>();
	        plays.add(0);
	        plays.add(1);
	        plays.add(2);
	        plays.add(3);
	        plays.add(4);
	        plays.add(5);
	        plays.add(6);
	        plays.add(7);
	        plays.add(8);
	        Collections.shuffle(plays);
	        return plays;
	    }
	    public Integer[] definition(final Integer[] newMatrix, final int key) {
	        int position = 0;
	        switch (key) {
	            case 8: {
	                for (int i = 0; i < 9; ++i) {
	                    if (newMatrix[i] == 0) {
	                        position = i;
	                    }
	                }
	                for (int i = 0; i < 9; ++i) {
	                    if (position == i) {
	                        if (i >= 0 && i <= 2) {
	                            System.out.println("Movement do not allowed\n");
	                        }
	                        else {
	                            newMatrix[i] = newMatrix[i - 3];
	                            newMatrix[i - 3] = 0;
	                        }
	                    }
	                }
	                break;
	            }
	            case 2: {
	                for (int i = 0; i < 9; ++i) {
	                    if (newMatrix[i] == 0) {
	                        position = i;
	                    }
	                }
	                for (int i = 0; i < 9; ++i) {
	                    if (position == i) {
	                        if (i >= 6 && i <= 8) {
	                            System.out.println("Movement do not allowed\n");
	                        }
	                        else {
	                            newMatrix[i] = newMatrix[i + 3];
	                            newMatrix[i + 3] = 0;
	                        }
	                    }
	                }
	                break;
	            }
	            case 4: {
	                for (int i = 0; i < 9; ++i) {
	                    if (newMatrix[i] == 0) {
	                        position = i;
	                    }
	                }
	                for (int i = 0; i < 9; ++i) {
	                    if (position == i) {
	                        if (i == 0 || i == 3 || i == 6) {
	                            System.out.println("Movement do not allowed\n");
	                        }
	                        else {
	                            newMatrix[i] = newMatrix[i - 1];
	                            newMatrix[i - 1] = 0;
	                        }
	                    }
	                }
	                break;
	            }
	            case 6: {
	                for (int i = 0; i < 9; ++i) {
	                    if (newMatrix[i] == 0) {
	                        position = i;
	                    }
	                }
	                for (int i = 0; i < 9; ++i) {
	                    if (position == i) {
	                        if (i == 2 || i == 5 || i == 8) {
	                            System.out.println("Movement do not allowed\n");
	                        }
	                        else {
	                            newMatrix[i] = newMatrix[i + 1];
	                            newMatrix[i + 1] = 0;
	                        }
	                    }
	                }
	                break;
	            }
	        }
	        return newMatrix;
	    }
	    @Override
	    public String toString() {
	        return "MatrixEightEntity [idMatrix=" + this.idMatrix + ", a11=" + this.a11 + ", a12=" + this.a12 + ", a13=" + this.a13 + ", a21=" + this.a21 + ", a22=" + this.a22 + ", a23=" + this.a23 + ", a31=" + this.a31 + ", a32=" + this.a32 + ", a33=" + this.a33 + "]";
	    }
	}
